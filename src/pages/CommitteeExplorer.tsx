...
export default function CommitteeExplorer() {
  ...
  const countryBreakdown = useMemo(() => {
    ...
    const counts: Record<string, number> = {};
    for (const m of members) {
      counts[m.countryCode] = (counts[m.countryCode] || 0) + 1;
    }
    return Object.entries(counts)
  ...
  if (isLoading) return <LoadingSpinner message="Loading committees..." />;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Explore parliamentary committees and their members
        </p>
      </div>
          ...
          <div className="mt-4 bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto">
              {filteredCommittees.map((c) => (
                <button
                  key={c.shortName}
                  onClick={() => setSearchParams({ selected: c.shortName })}
                  className={`w-full text-left px-4 py-3 flex items-center justify-between gap-2 transition-colors ${
                    selectedCommittee === c.shortName
                      ? 'bg-primary-50 border-l-4 border-primary-500'
                      : 'hover:bg-slate-50'
                  }`}
                >
                  <div className="min-w-0">
                    <span className="font-semibold text-slate-900 text-sm">{c.shortName}</span>
        ...
        <div className="lg:col-span-2 space-y-6">
          {selectedCommittee ? (
            <>
              ...
              {membersLoading ? (
                ...
                <>
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 p-6">
                    <h3 className="font-semibold text-slate-900 mb-4">Political Group Composition</h3>
                    <div className="h-48">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
              ...
              <h3 className="font-medium text-slate-600">Select a committee</h3>
              <p className="text-sm text-slate-400 mt-1">
                Choose a committee from the list to see its members and composition
              </p>
            </div>
          )}
        </div>
